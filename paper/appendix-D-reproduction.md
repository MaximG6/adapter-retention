...
| directory | built with | consumed by | difference |
|---|---|---|---|
| `paper/figures/` | default | the technical-report PDF, and the README | each figure carries its own title and caption text |
| `paper/figures-paper/` | `AR_FIG_PAPER=1` | the LaTeX build, `paper/tex/` | in-figure headers suppressed, because LaTeX sets the captions |

Both are regenerated by the same three scripts; the environment variable is the only
input that differs. Rebuild both with:
...
raw records, in whichever mode it is run. A figure that disagrees with the data fails the
...
## D.7a Verified end to end on a fresh clone
...
| `audit_draft_numbers.py --strict` | **106/106**, exit 0 |
