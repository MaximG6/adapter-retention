<!-- Generated by analysis/gen_readme.py from results/raw/**. Do not edit by hand: run `PYTHONPATH=src python analysis/gen_readme.py --write`. -->
# Adapter Retention Under Post-Training Quantization

Fine-tune a model with LoRA, merge the adapter in, quantize to 4-bit for deployment. How much of the fine-tune survives?

Almost none of the weight update does: the realised change points 13.8% of the way toward the intended one. The behaviour survives anyway, with no detectable loss at INT4 g128. This repository has the paper, the code, the raw records and the full lab notebook.
...
| **[Lab notebook](EXPERIMENTS.md)** (65 entries) | Append-only, including the experiments that failed, the ones that were misconfigured and the ones that answered nothing. Read the [supersession index](EXPERIMENTS.md#-supersession-index--read-before-quoting-any-number-from-this-file) before quoting any number from it. |
...
PYTHONPATH=src python -m pytest -q                      # 243 passed
