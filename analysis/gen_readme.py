...
def main() -> int:
    ...
    #: Two regimes, named separately, because the opening line and the numbers section
    #: are answering different questions. The opening sentence sits directly above
    #: Figure 1 and beside a behavioural claim, so it takes the DEPLOYMENT regime the
    #: figure and the behavioural runs both use -- quoting the fixed-grid value there
    #: printed 13.9% under a figure reading 13.8%. The numbers section keeps the
    #: fixed-grid cosine the paper leads with. They agree to 0.8% and differ in the
    #: first decimal of a percentage, which is exactly the size of gap a reader notices
    #: and no gate would have.
    cos_taboo = mean([coss[a] for a in coss if "taboo" in a])
    cos_taboo_deploy = mean([mean([r["cosine"] for r in v])
                             for a, v in p0_adp.items() if "taboo" in a])

    xl = exp_links()
    L: list[str] = []
    a = L.append
    # The generation warning is an HTML comment, not prose. It is addressed to whoever
    # edits this repository, and the first screen belongs to whoever is deciding whether
    # to read further. Same for the status line that used to sit here: "manuscript
    # drafted, Phase 2 not started" described a project in progress, and Phase 2 was a
    # decision with a recorded falsifier rather than an omission.
    a("<!-- Generated by analysis/gen_readme.py from results/raw/**. Do not edit by "
      "hand: run `PYTHONPATH=src python analysis/gen_readme.py --write`. -->")
    a("# Adapter Retention Under Post-Training Quantization")
    a("")
    a("Fine-tune a model with LoRA, merge the adapter in, quantize to 4-bit for "
      "deployment. How much of the fine-tune survives?")
    a("")
    a(f"Almost none of the weight update does: the realised change points "
      f"{cos_taboo_deploy * 100:.1f}% of the way toward the intended one. The behaviour "
      f"survives anyway, with no detectable loss at INT4 g128. This repository has the "
      f"paper, the code, the raw records and the full lab notebook.")
    a("")
    ...
    a(f"**At INT4 with group size 128 — the standard deployment configuration — the "
      f"adapter's intended weight update arrives with cosine {cos_taboo:.2f} to itself, "
      f"and {behav:.1f}% of the adapter's trained elicitation capability is retained.** "
      f"Both measured on the same six adapters, quantized the same way. Capability is "
      f"one side of behaviour; the other is the trained constraint, and it moves — the "
      f"adversarial leak rate falls {leak_fall:.1f} points, [+{leak_lo:.1f}, "
      f"+{leak_hi:.1f}].")
    a("")
