...
Standing context for this repository. Read `docs/PROJECT-EXECUTION-PLAN-v2.md` for full detail; this file is the operating agreement.

This file stays at the repository root because Claude Code loads it from there. Every other process document lives in `docs/` — see `docs/README.md`.
...
falsifier. Manuscript drafted in `paper/`, 65 entries in `EXPERIMENTS.md`, both PDFs
...
`EXP-NNN` headings in the notebook. It read 31 against 58 until then, because every
gate stopped at the repository's public face and this is the file that decides what
goes into it. **If you add a notebook entry, this number goes stale and the build
fails** — which is the point, and it caught this sentence at 58 against 59 within
...
numbers in the README (§7.8, EXP-022). If you are reading it and it disagrees with
`EXPERIMENTS.md`, `EXPERIMENTS.md` is right.
...
7. **Repo layout.**
8. **Prior work and how this differs.** Short, honest, links `docs/PRIOR_ART.md`.
...
1. **Prior art before code.** `docs/PRIOR_ART.md` must exist and be filled before implementing anything. LoftQ and QA-LoRA are both motivated by the LoRA-quantization interaction; if the numerical result is fully covered, we pivot to the behavioral framing that day. Do not skip this to start coding.
...
Planned layout. Where the tree diverged from it, the tree is right and the
divergence is noted inline.

```
README.md          public face, generated by analysis/gen_readme.py
EXPERIMENTS.md     append-only lab notebook
LICENSE            MIT
CLAUDE.md          this file; stays at root so Claude Code loads it
docs/              process record: the plan, prior art, outline, read-through
  PRIOR_ART.md              day 1, before code
  PROJECT-EXECUTION-PLAN-v2.md
  PREREGISTRATION.md        before the Phase 2 sweep -- NOT WRITTEN, Phase 2 not started
  VALIDATION.md             the 20-trajectory manual audit -- NOT WRITTEN, see EXP-031
src/ar/
  ...
  predict.py     ar.predict, the shipped tool
  device.py      capability-based device resolution
  manifest.py    environment capture
  schema.py      Record -- normative, not imported at runtime
  runner.py      Phase 2 ATP wrapper -- NOT WRITTEN, Phase 2 not started
  (config.py was planned for pydantic schemas; they went into schema.py instead)
...
vendor/ATP/      submodule, Phase 2 only -- NOT PRESENT
patches/         -- NOT PRESENT, no third-party code was modified
paper/
```
